...
class ProductionLine:
    ...
    def build_simulation(self, max_time):
        ...
        count = 0
        while True:
            exp_time = exponential(5)
            norm_time = normal(4,0.5)
            if time + exp_time + norm_time > max_time:
                break
            self.env.process(self.piece('Piece %d' % count, time + exp_time, norm_time))
            time += exp_time
            count += 1
...
plt.legend([f"servers = {servers}, max time = {max_time}"], loc ="upper right")
